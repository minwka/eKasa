...
namespace eKasa.Library.Config {
	static public class SettingsManager<T> {
		...
		static public void Restore(ref T settingsModel, string filePath) { if (File.Exists(filePath)) settingsModel = JsonSerializer.Deserialize<T>(File.ReadAllText(filePath)); }
	}
}